...
Picking up from where we left off last time, we’ll just going to look at the callback functions for each example… until we get to those examples that are more complex.
...
a) In theory, you can have any number of dash pattern numbers you want.
b) Numbers can be with or without decimals. A Context will treat 25 the same way it treats 25.0, but of  course, you can go sub-pixel with fractions such as 25.3 or whatever.
c) Changing the line caps with the setLineCap() function will affect the look of a dash pattern to the point where your dash pattern numbers may seem like they’re being ignored.
...
The callback for filing a rectangle looks like this:

{% highlight d %}
	bool onDraw(Scoped!Context context, Widget w)
	{
		context.setSourceRgb(0.541, 0.835, 0.886);		context.rectangle(150, 100, 340, 170);		context.fill();
